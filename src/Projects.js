          ...
          {projectsData.map((project, index) => {
            ...
            return (
              <div className='p-6 rounded-2xl border-[6px] border-slate-200 bg-gray-200 transition duration-500  hover:shadow-xl hover:border-slate-100' key={index}>
                ...
                <div className='mb-4 text-sm text-slate-400'>{description.length >= 95 ? <AdvReadMoreMore text={description} linesToShow={2} checkFor={50} color='#1e293b' btnStyles={{ color: '#6b7280' }} /> : description}</div>
                {published === '' ? (
