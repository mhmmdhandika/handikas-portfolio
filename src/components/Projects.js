      ...
      <div className="child-section">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {projectsData.map((project, index) => {
            ...
            return (
              <div
                className="p-6 rounded-2xl border-4 border-slate-400 bg-slate-100 transition duration-500  hover:shadow-xl hover:scale-105"
                key={index}
              >
                <img
                  src={image}
                ...
                {published === "" ? (
                  <button
                    className="block mb-4 text-sm text-slate-500 underline cursor-not-allowed"
                    disabled
                  >
                    Not yet published
                  </button>
